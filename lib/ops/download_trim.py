...
def download_and_trim_row(row, raw_dir: Path, out_dir: Path, ledger_path: Path,
    ...
    if out_wav.exists():
        append_ledger({
            "source":"audioset","youtube_id":ytid,"start":start,"end":start+dur,
            "label":label,"status":"skip","out_path":str(out_wav),
            "sr":sr,"cliplen":dur,"note":"exists"
        }, ledger_path)
        return

    # 1) yt-dlp로 원본 오디오 다운로드 (임시)
    tmp = raw_dir / f"{ytid}.%(ext)s"
    url = f"https://www.youtube.com/watch?v={ytid}"
    cmd_ytdlp = ["yt-dlp","-f","ba","-o", str(tmp), "--no-part","--no-progress", url]
    rc = _run(cmd_ytdlp)
    if rc != 0:
        append_ledger({
            "source":"audioset","youtube_id":ytid,"start":start,"end":start+dur,
    ...
    if got is None:
        append_ledger({
            "source":"audioset","youtube_id":ytid,"start":start,"end":start+dur,
            "label":label,"status":"fail","out_path":"", "sr":sr,"cliplen":dur,
            "note":"downloaded file not found"
        }, ledger_path)
        return

    # 2) ffmpeg로 트림→WAV(모노, 48k)
    cmd_ff = [
        "ffmpeg","-hide_banner","-loglevel","error",
        "-ss", str(start), "-t", str(dur),
        "-i", str(got),
        "-ac","1","-ar", str(sr), "-vn","-sn",
        str(out_wav)
    ]
    rc = _run(cmd_ff)
    ...
    if rc == 0 and out_wav.exists():
        append_ledger({
            ...
            "sr":sr,"cliplen":dur,"note":""
        }, ledger_path)
    else:
        append_ledger({
            "source":"audioset","youtube_id":ytid,"start":start,"end":start+dur,
            "label":label,"status":"fail","out_path":str(out_wav),
            "sr":sr,"cliplen":dur,"note":f"ffmpeg rc={rc}"
        }, ledger_path)
